...
#include <fstream>
#include <string>
#include "include/fireAndRainCell.hpp"

using namespace cadmium::celldevs;
using namespace cadmium;

std::shared_ptr<GridCell<fireAndRainState, double>> addGridCell(const coordinates & cellId, const std::shared_ptr<const GridCellConfig<fireAndRainState, double>>& cellConfig) {
	auto cellModel = cellConfig->cellModel;

	if (cellModel == "fireAndRain") {
		return std::make_shared<fireAndRain>(cellId, cellConfig);
	} else {
